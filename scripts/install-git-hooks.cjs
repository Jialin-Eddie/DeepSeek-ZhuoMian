#!/usr/bin/env node
/**
 * 安装 git pre-push 钩子：每次 `git push` 前强制运行安全检查
 * （npm run security），不过关自动中止 push。
 *
...
# dsh-zhuomian pre-push security gate（由 install-git-hooks.cjs 自动安装）
# 紧急跳过：SKIP_SECURITY_CHECK=1 git push（危险，仅限应急）
if [ -n "$SKIP_SECURITY_CHECK" ]; then
  echo "[security] WARNING: SKIP_SECURITY_CHECK set — 本次 push 跳过安全检查（危险！）"
  exit 0
fi
node "${checkScript}"
status=$?
if [ $status -ne 0 ]; then
  echo "[security] ✗ 安全检查未通过（exit=$status），push 已中止。请修复后重试。"
  exit 1
fi
echo "[security] ✓ 安全检查通过"
exit 0
`
...
console.log('done. 之后每次 git push 都会自动先跑 security check。')
